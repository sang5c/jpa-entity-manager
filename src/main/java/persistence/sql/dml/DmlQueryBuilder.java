...
public class DmlQueryBuilder {
    ...
    public String buildUpdateQuery(String tableName, ColumnClause id, List<ColumnClause> columnClauses) {
        return """
                update %s\s
                set %s\s
                where %s
                ...
                .formatted(
                        tableName,
                        equalityExpressions(columnClauses),
                        id.equalityExpression()
                );
    }

    private String equalityExpressions(List<ColumnClause> columnClauses) {
        return columnClauses.stream()
                .map(ColumnClause::equalityExpression)
